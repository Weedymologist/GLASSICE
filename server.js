// server.js - v38.0 (Visual Consistency Update)
...
// --- MODIFIED: The Art Director AI now uses faction visual descriptions for consistency ---
async function generateShotDescription(narration, artStyle, faction1Visuals, faction2Visuals) {
    console.log(`[AI-ARTIST] Generating shot description with style: ${artStyle}`);
    const system_prompt = `You are a master Art Director and Prompt Engineer for a high-end text-to-image AI. Your sole purpose is to convert a piece of narrative text into a single, vivid, and detailed 'shot_description'.

Your generated prompts MUST be a comma-separated list of descriptive keywords and phrases.

CRITICAL INSTRUCTIONS:
1.  **Maintain Visual Consistency:** You will be given key visual descriptions for each faction. These are the most important rules. Any characters or units mentioned MUST adhere to these descriptions. For example, if Faction 1 "wears chrome armor", they must always be depicted in chrome armor.
2.  **Adhere to the Art Style:** The final image MUST conform to the user-selected Art Style.
3.  **Analyze the Narration:** Determine the most important visual elements from the story text.
4.  **Construct the Prompt:** Build a powerful prompt using cinematic terms, dramatic lighting, and quality keywords.
5.  **Output Format:** The final output MUST be a single JSON object with ONE key: "shot_description".`;
...
async function generateImage(shotDescription) {
    if (!shotDescription || !STABILITY_API_KEY) { return null; }
    console.log(`[IMAGE] Generating image...`);
    const formData = new FormData();
    formData.append('prompt', shotDescription);
    formData.append('aspect_ratio', '16:9');
    formData.append('negative_prompt', 'ugly, deformed, disfigured, blurry, low quality, duplicate, bad anatomy, extra limbs, mutated hands, poorly drawn hands, poorly drawn face, text, watermark, signature');
    try {
        const response = await fetch("https://api.stability.ai/v2beta/stable-image/generate/core", { method: 'POST', headers: { ...formData.getHeaders(), "authorization": `Bearer ${STABILITY_API_KEY}`, "accept": "image/*" }, body: formData, });
    ...
    } catch (error) { console.error("[IMAGE] Stable Diffusion generation failed:", error.message); return null; }
}

// (All other AI functions and boilerplate code remain the same)
...
app.post('/api/dynamic-narrative/start', async (req, res) => {
    // MODIFIED: Destructure the new visual description properties
    const { gameSettingPrompt, playerSideName, opponentSideName, gameMode, artStyle, faction1Visuals, faction2Visuals } = req.body;
    const sceneId = `scene_${Date.now()}`;
    try {
        const narration = await fetchNarration(gameSettingPrompt);
        // MODIFIED: Pass the visual descriptions to the image generation pipeline
        const [shotDescription, audioB64] = await Promise.all([generateShotDescription(narration, artStyle, faction1Visuals, faction2Visuals), generateAudio(narration)]);
        const imageB64 = await generateImage(shotDescription);
        
        activeScenes[sceneId] = {
            history: [{ role: 'system', content: 'The scene begins.' }, { role: 'assistant', content: narration }],
            playerSideName,
            opponentSideName,
            gameMode,
            artStyle: artStyle || "Cinematic Realism",
            faction1Visuals: faction1Visuals || "", // NEW: Save visual descriptions
            faction2Visuals: faction2Visuals || "", // NEW: Save visual descriptions
            playerHP: 100,
            opponentHP: 100,
            playerEffects: [],
            opponentEffects: []
...
app.post('/api/dynamic-narrative/:sceneId/turn', async (req, res) => {
    ...
    try {
        let responsePayload = {};
        // MODIFIED: Pass all context to the image generation pipeline
        ...
        if (scene.gameMode === 'competitive') {
            ...
            const [shot, audio] = await Promise.all([generateShotDescription(combatResult.narration, artStyle, faction1Visuals, faction2Visuals), generateAudio(combatResult.narration)]);
            ...
            const [shot, audio] = await Promise.all([generateShotDescription(finalNarration, artStyle, faction1Visuals, faction2Visuals), generateAudio(finalNarration)]);
            const image = await generateImage(shot);
